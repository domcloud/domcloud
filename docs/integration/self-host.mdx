...
Tools, libs, services:
```
dnf install certbot clang cmake gcc-c++ git ncdu htop iftop ipset jq lsof make nano rsync sendmail strace tar time vim wget xz
dnf install {libcurl,libffi,libsqlite3x,libtool-ltdl,nettle,openldap,postgresql-server,passenger,perl,readline,xmlsec1,xmlsec1-openssl}-devel libmd libreport-filesystem mesa-libGL perl-{DBD-Pg,macros,Crypt-SSLeay}
dnf install earlyoom fail2ban-server iptables-services postfix postgresql-server postgresql-contrib
```
