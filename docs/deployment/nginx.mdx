...
Static file caching is enabled by default for media files and CSS + JS files. This is done by sending `Expires` header to the browser, so browser will cache your static files without checking it with our server until the specified time.

The default settings is 7 days. To disable it, set `expires` to `off`:
...
Note that this don't disables E-Tag header -- so when files unchanged, it still send `304 Not Modified`. 
