...
This also sets the `node` (and `npm`, `yarn`, etc.) to the right version in CLI/SSH. This installation is powered by [`webi`](https://webinstall.dev/node). All given node version is downloaded in binary/compiled version from [the official registry](https://nodejs.org/download/release) to keep switching version fast.
...
The installer will try to download binary/compiled version from [community builds](https://github.com/indygreg/python-build-standalone) if requested version is available to keep switching version fast.
...
By default it's Ruby 3.0 provided from RHEL system packages. This installation is powered by [rvm](https://rvm.io). 
The installer will try to download binary/compiled version from [RVM builds](https://rvm_io.global.ssl.fastly.net/binaries/centos/9/x86_64/) if requested version is available to keep switching version fast.
...
This installation is powered by [`webi`](https://webinstall.dev/bun).
...
This installation is powered by [Adoptium Project](https://adoptium.net/). Only latest build from them in each major version (as low as `jdk 8`) is available to download using this feature.
...
This installation is powered by [Microsoft .NET Core builds](https://dotnet.microsoft.com/en-us/download/dotnet). Only versions provided by them is available to install.
...
This installation is powered by [`webi`](https://webinstall.dev/zig). 
