...
Database is an **opt-in**. You have to enable it via the deployment script. See the section for enabling [MariaDB](./deploy.mdx#mysql) or [PostgreSQL](./deploy.mdx#postgresql).
...
You can't create or drop database from PhpMyAdmin, PhpPgAdmin or any database client. You have to do that from Webmin UI or deployment script.
