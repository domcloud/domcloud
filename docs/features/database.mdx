...
You can't create or drop database with PhpMyAdmin, PhpPgAdmin, phpRedisAdmin or with any external database client. You have to do that from Webmin UI or [deployment script](../deployment/deploy.mdx#features).

Our valkey instance is [shared across the server with ACL](#valkey-acl-rules) and managed outside webmin interface.
...
PostgreSQL and MariaDB permits remote access by default, while Valkey don't. We do not recommend you to manage valkey directly, though you still can use PhpRedisAdmin.
