...
Our server serves HTTP and HTTPS traffic using NGINX. NGINX is used to serve static files and proxy requests to the app daemon. Because it operates for all users on the server, only a limited set of features are configurable. It's only configurable using the [runner](./runner.mdx#nginx).
...
The left side is the configurable options, and the right side is the current configuration applied in NGINX. The configurable option is written in YAML format, which when saved, applied as a new configuration configured via [the runner](./runner.mdx#nginx).
...
The [`ssl`](./runner.mdx#ssl-1) option has three options: `always`, `on`, `off`. The `always` option will force the server to redirect all HTTP traffic to HTTPS. The default value for `ssl` is `on`.

## Configure NGINX for PHP

The [`fastcgi`](./runner.mdx#fastcgi) option has three options: `always`, `on`, `off`. The default value for `fastcgi` is `off`.
