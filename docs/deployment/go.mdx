...
For existing golang apps, use this deployment script. This project must have a `go.mod` initialized.
...
To switch golang version use feature syntax like `go stable`, `go beta`, `go v1.19`, etc. Check your current go version using `go version` in SSH.
...
Please use a proper logging mechanism such as [the standard log library](https://pkg.go.dev/log) then write it to a log file, or any other solution that suits you.
