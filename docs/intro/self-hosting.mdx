...
Let's say you have a `mysite.com` domain, we'll assign bridge API to `bridge.mysite.com` by setting A+AAAA/CNAME record to your VPS. The `bridge.` subdomain is important since you can want to host an actual website to `mysite.com` not the internal bridge API.
