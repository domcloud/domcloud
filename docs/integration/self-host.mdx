...
pssh -t 0 -iPh ~/hosts 'bash -c "cat <<EOF > /output/to/file
This is line 1
This is line 2
EOF"'
```
...
for bin in "psql" "pg_dump" "pg_dumpall" "pg_restore" "pg_isready"; do
...
[Service]
Type=forking
Environment="UNITD_OPTIONS=--control unix:/var/run/unit/control.sock --log /var/log/unit/unit.log --pid /var/run/unit/unit.pid"
