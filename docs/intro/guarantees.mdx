...
We limit RAM for any instance up to 2GB of RAM for Tier I servers and 512MB RAM for Tier II servers. While this is particularly high standard for shared hosting like us, it's done that way since we want to only allocate high memory for temporary things like compiling your apps.

Never allocate RAM/CPU usage beyond necessary. We have an alert if the CPU usage is too high continously. If it does happen, we have to kill the process forcibly so it doesn't impact other websites for too long. We could terminate an account if it comes again repeatedly.
