...
Most ports are blocked except ephemeral ports (32000 - 65535). You can use them for local development inside the server, including running a custom TCP listener without going though NGINX.
...
```bash
npm run dev -- --host $(hostname -I | cut -d " " -f1) --port $(shuf -n 1 -i 32000-65535)
```
