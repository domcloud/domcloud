...
```bash
echo "passenger-config restart-app ~" > /usr/local/bin/restart && chmod 755 $_
```
...
Tools, libs, services:
```
dnf install certbot clang cmake gcc-c++ git ncdu htop iftop ipset jq lsof make nano rsync sendmail strace tar time vim wget yarn xz
dnf install {libcurl,libffi,libsqlite3x,libtool-ltdl,nettle,openldap,postgresql-server,passenger,perl,readline,xmlsec1,xmlsec1-openssl}-devel libmd libreport-filesystem mesa-libGL perl-DBD-Pg perl-macros
