...
Outbound connection is for outgoing traffic to the internet. It's allowed for all ports and protocols by default, except port `25`, it's blocked to prevent SMTP / Email abuse. If you need to send emails, you can use a third-party email relay services that uses a more secure `587` port or even with HTTP API.
