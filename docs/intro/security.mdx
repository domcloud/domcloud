...
To aid in installing additional binaries and tools we have plenty of [deployment scripts](../deployment/deploy.mdx) that install things in your `$HOME` folder. Some action that requires sudo such as configuring NGINX or DNS records are also handled by deployment scripts or our panel Webmin.

The deployment system also has some shortcut to install language compilers, such as Node.JS, Python, Rust, Go, and many more. Refer to [deployment section](../deployment/index.mdx). These will be installed to the user's home dir (usually `~/.local/bin`) so it's isolated with system-wide software.

No matter your system requirement is, try to find a way to do it without sudo. Again, we are not giving you sudo access to cloud servers because the shared hosting nature. Our system also designed to follow shared hosting best practices and it's protected behind sudo. You almost can't broke our system whatever you're trying to do. 
