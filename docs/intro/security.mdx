...
The deployment system also has some shortcut to install language compilers, such as Node.JS, Python, Rust, Go, and many more. Refer to [its `features` section](../deployment/deploy.mdx#features). These will be installed to the user's home dir (usually `~/.local/bin`) so it's isolated with system-wide software.

No matter your requirement is, we are not giving you sudo access to cloud servers because the shared hosting nature. If you still insist, please put your system inside [Docker](../deployment/docker.mdx), or set up a [self-hosted server](./self-hosting.mdx).
