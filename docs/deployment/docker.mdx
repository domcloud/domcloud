...
Enable docker with putting this to [deployment script](./deploy.mdx):
...
nginx:
  ...
  passenger:
    enabled: on
    app_start_command: proxfix docker compose up
    env_var_list:
    - NOHUP=1
    - TARGET=127.X.X.X:8080
```

Note `TARGET` envar is where docker binds to. To know what is this value, refer to `proxy_pass` value before NGINX set.
