...
Popular PHP recipes is [CodeIgniter](https://codeigniter.com/) and [Laravel](https://laravel.com/). Please read our [Runner's Guide](../features/runner.mdx) first if you haven't.
...
The [`fastcgi`](../features/runner.mdx#fastcgi) option has three options: `on`, `off`, `always`. The difference between three options:
