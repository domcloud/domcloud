...
After that, change the website's domain name in DOM Cloud's Portal (Select a website -> Admin -> Change Domain). Then, check your website connectivity in menu Check -> Check Connection. If it shown like an image below, then your set up is done correctly.
...
When you create a website with custom domain (other than `*.domcloud.dev`), a `dns` service is activated from [the deployment script](./deploy.mdx#dns), it's activated automatically by our Portal UI.
...
If you experience infinite redirects when using Cloudflare, disable its proxy <br/> (under `Proxy status`).
...
```txt title="$> nslookup sga.domcloud.id nsp.domcloud.co"
