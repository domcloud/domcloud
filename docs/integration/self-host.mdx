...
{libcurl,libffi,libsqlite3x,libtool-ltdl,libyaml,nettle,openldap,passenger,perl,readline,xmlsec1,xmlsec1-openssl}-devel \
libmd libreport-filesystem mesa-libGL perl-{DBD-Pg,DBD-mysql,macros,Crypt-SSLeay,Text-ASCIITable,IO-Tty} {langpacks,glibc-langpack}-en \
earlyoom fail2ban-server iptables-services postfix mariadb-server wbm-virtual-server wbm-virtualmin-{nginx,nginx-ssl}
```
