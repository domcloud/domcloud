...
dnf install php{74,80,81,82,83}-php-{bcmath,cli,common,devel,fpm,gd,imap,intl,mbstring,mysqlnd,opcache,pdo,pecl-imagick,pecl-mongodb,pecl-redis,pecl-zip,pgsql,process,sodium,soap,xml} composer
...
earlyoom fail2ban-server iptables-services postfix
```
...
```
dnf module reset node
dnf module enable node:20
yum update
```

Ruby (Use newer module version):
```
